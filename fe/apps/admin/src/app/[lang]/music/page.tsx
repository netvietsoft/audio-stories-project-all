...
// < 1 giờ -> mm:ss ; >= 1 giờ -> dd:hh:mm:ss (ngày:giờ:phút:giây).
...
export default function AdminMusicPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const meta = (response.data as any)?.data?.meta ?? (response.data as any)?.meta;
  ...
  // Bỏ popup: form hiển thị inline dưới trang -> tự cuộn tới khi mở.
  useEffect(() => {
    if (isModalOpen) {
      document
        .getElementById("music-edit-panel")
          ...
          <table className="w-full min-w-[920px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                ...
                items.map((row) => (
                  <tr key={row.id} onClick={() => openEditModal(row)} className="cursor-pointer hover:bg-slate-50/60">
                    ...
                    <td className="px-6 py-4 text-sm font-semibold text-slate-900">
                      {row.title}
                      <div className="mt-1 flex max-w-[320px] flex-col gap-0.5 text-[11px] font-normal">
                        <span className="select-all font-mono text-slate-400" title="ID bài hát (click để bôi đen)">
                          ID: {row.id}
                        </span>
                        {(row as any).audioUrl ? (
                          <a
                            href={(row as any).audioUrl}
                            target="_blank"
                            rel="noreferrer"
                            className="truncate text-indigo-600 hover:underline"
                            title={(row as any).audioUrl}
                          >
                            🔗 {(row as any).audioUrl}
                          </a>
                        ) : (
                          <span className="text-slate-400">(chưa có file audio)</span>
                        )}
                      </div>
                    </td>
