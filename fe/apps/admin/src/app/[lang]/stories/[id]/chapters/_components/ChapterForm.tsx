...
export const ChapterForm = ({ initialData, selectedLocale = 'vi', onSubmit, onCancel, isLoading }: ChapterFormProps) => {
    ...
    const audioInputViRef = useRef<HTMLInputElement>(null);
    const audioInputEnRef = useRef<HTMLInputElement>(null);
    ...
    // Xoá file cũ trên R2 qua BE: POST /upload/delete (ảnh + audio dùng chung endpoint,
    // BE tự bỏ qua nếu URL không thuộc R2). adminApiClient đã kèm token admin.
    const deleteOldThumbnail = async (thumbnailUrl: string) => {
        if (!thumbnailUrl) return;
        try {
            await adminApiClient.post('/upload/delete', { url: thumbnailUrl });
        } catch (error) {
    ...
    const deleteOldAudio = async (audioUrl: string) => {
        if (!audioUrl) return;
        try {
            await adminApiClient.post('/upload/delete', { url: audioUrl });
        } catch (error) {
            console.error('Failed to delete old audio:', error);
        }
    };

    // Đọc thời lượng audio từ file cục bộ (trước khi upload) cho field audioDuration.
    const readAudioDurationFromFile = (file: File) => {
    ...
    // Upload thẳng ảnh bìa lên R2 qua BE: POST /upload/image (thay UploadThing — không cần token).
    const handleThumbnailFileSelect = async (file: File | null | undefined) => {
        if (!file) return;
        ...
        try {
            ...
            else setError('thumbnailUrl' as any, { type: 'upload', message: 'Lỗi tải ảnh: không nhận được URL từ máy chủ.' });
        } catch (err: any) {
            setError('thumbnailUrl' as any, { type: 'upload', message: `Lỗi tải ảnh: ${err?.response?.data?.error?.message || err?.message || 'Vui lòng thử lại.'}` });
        } finally {
            setIsUploadingThumbnail(false);
        }
    };

    // Upload thẳng file audio lên R2 qua BE: POST /upload/audio (field `file`, folder=chapters)
    const handleAudioFileSelect = async (lang: Locale, file: File | null | undefined) => {
        if (!file) return;
        const audioFieldName = lang === 'vi' ? 'audioUrlVi' : 'audioUrlEn';

        if (lang === 'vi') setIsUploadingAudioVi(true);
        if (lang === 'en') setIsUploadingAudioEn(true);

        // Đọc duration từ file cục bộ trước (đáng tin cậy nhất)
        ...
        try {
            ...
            const res = await adminApiClient.post('/upload/audio', formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            ...
            if (uploadedUrl) {
                ...
                setError(audioFieldName as any, { type: 'upload', message: 'Lỗi tải audio: không nhận được URL từ máy chủ.' });
            }
        } catch (error: any) {
            const message = error?.response?.data?.message || error?.message || 'Tải audio thất bại';
            setError(audioFieldName as any, { type: 'upload', message: `Lỗi tải audio: ${message}` });
        } finally {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const chaptersRaw = unwrapList<any>(res.data);
            ...
            <div className={`flex flex-col gap-4`}>
                ...
                <div className="space-y-3">
                    ...
                    <div className="relative group">
                        ...
                        />
                        <label
                            htmlFor={`audio-upload-${lang}`}
                            className="w-full min-h-[130px] bg-slate-50 border-2 border-dashed border-slate-200 rounded-[20px] flex flex-col items-center justify-center gap-2.5 text-slate-700 hover:border-indigo-300 transition-all cursor-pointer"
                        >
                                ...
                                {watch('thumbnailUrl') ? (
                                    ...
                                    <>
                                        <input
                                            ...
                                            onChange={(e) => {
                                                const file = e.target.files?.[0];
                                                void handleThumbnailFileSelect(file);
                                                e.target.value = '';
                                            }}
                                        />
                                        <button
                                            ...
                                            className="mx-auto flex h-[100px] w-[100px] items-center justify-center rounded-[24px] border-2 border-dashed border-slate-300 bg-white transition hover:border-indigo-400 hover:bg-indigo-50/40 disabled:opacity-70"
                                        >
