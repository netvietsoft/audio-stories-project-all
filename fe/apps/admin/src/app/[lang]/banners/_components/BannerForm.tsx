...
export default function BannerForm({ initialData, selectedLocale, isLoading, onSubmit, onCancel }: BannerFormProps) {
  ...
  // Upload ảnh banner lên R2 qua BE POST /upload/image (thay UploadThing).
  const handleBannerImageSelect = async (file: File | null | undefined) => {
    if (!file) return;
    setIsUploadingImage(true);
    try {
      const formData = new FormData();
      formData.append('file', file);
      const res = await apiClient.post('/upload/image', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      const data = res.data as { data?: { url?: string }; url?: string } | undefined;
      const uploadedUrl = data?.data?.url ?? data?.url ?? '';
      if (uploadedUrl) setValue('imageUrl', uploadedUrl, { shouldValidate: true });
              ...
              {previewUrl ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className="flex min-h-[170px] w-full flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 transition hover:border-indigo-400 hover:bg-indigo-50/40 disabled:opacity-60"
                  >
                    {isUploadingImage ? (
                      <>
