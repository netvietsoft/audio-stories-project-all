...
| `reader_screen.dart` | Đọc chương | `/reader/:id?ch=` | **File lớn nhất.** Chỉnh cỡ chữ/nền (light/sepia/dark)/màu chữ (8 màu); **read-along** (highlight câu + auto-scroll theo `timing` của chương); panel khoá → `showUnlockSheet`; cuối chương Comment/Support/Share; Prev/Next. |
