...
export class VietQRService {
  ...
  /**
   * Huỷ đơn VietQR còn PENDING đã quá hạn (expiresAt < now). Chỉ chạy trong vai
   * 'scheduler' (ScheduleModule chỉ init ở đó; @Cron inert ở api/worker).
   * Sau khi CANCELLED, webhook tới muộn sẽ không khớp `status: PENDING` nên
   * không cộng nhầm Pulse.
   */
  ...
  async cancelExpiredOrders() {
    ...
    const result = await this.prisma.payment.updateMany({
      ...
      data: {
        status: 'CANCELLED',
        failedAt: now,
        failureMessage: 'Order expired',
      },
    });

    if (result.count > 0) {
      this.logger.log(`Cancelled ${result.count} expired VietQR order(s)`);
    }
  }
}
