 ...
 *
 * Yêu cầu:
 * - Web app đang chạy tại WEB_URL (default: http://localhost:3001)
...
test.describe('Public Pages – Không cần đăng nhập', () => {
  test('✅ Trang chủ public (/vi hoặc /vi/story) có thể truy cập', async ({ page }) => {
    const res = await page.goto(`${WEB_URL}/vi/story`, { waitUntil: 'networkidle' });
...
test.describe('Audio Proxy – Thẻ <audio> phải dùng backend proxy', () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await page.goto(`${WEB_URL}/vi/story/${storySlug}`, { waitUntil: 'networkidle' });
