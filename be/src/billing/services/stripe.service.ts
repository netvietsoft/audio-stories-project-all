...
export class StripeService {
  ...
  async verifyAndProcessPayment(userId: string, sessionId: string) {
    ...
    try {
      ...
      // Check if payment already processed. Use the Checkout Session id (stable &
      // identical across the verify-payment and webhook paths) as the idempotency key.
      ...
      // Process the payment. (provider, providerPaymentId) is UNIQUE in the DB, so a
      // concurrent webhook crediting the same Checkout Session hits P2002 and the whole
      // transaction (incl. the balance increment) rolls back — preventing a double credit.
      try {
        ...
        if (e?.code === 'P2002') {
          this.logger.log('Payment already processed (unique constraint) — skipping double credit');
