...
export class PasswordService {
  ...
  /**
   * Đổi mật khẩu khi đã đăng nhập: xác minh mật khẩu hiện tại rồi đặt mật khẩu mới.
   * Trả về userId để caller (AuthService) thu hồi các phiên refresh khác nếu muốn.
   */
  async changePassword(
    ...
    if (!user || !user.passwordHash) {
      // Tài khoản chỉ có Google OAuth (chưa đặt mật khẩu local).
