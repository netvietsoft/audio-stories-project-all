...
export default function StoryChapterManager({ storyId }: StoryChapterManagerProps) {
  ...
  const handleGoBack = () => {
    ...
    if (storyId) {
      router.push(`/${currentLang}/stories/${storyId}`);
      return;
    }

    router.push(`/${currentLang}/stories`);
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const story = unwrapData<any>(storyRes.data) ?? {};
  ...
  // Mở trang sửa chương ở TAB MỚI để quản lý/sửa nhiều chương cùng lúc
  // (giữ nguyên danh sách chương ở tab hiện tại).
  const handleEdit = (chapterId: string) => {
    const url = `/${currentLang}/stories/${storyId}/chapters/${chapterId}/edit`;
            ...
            <div className="divide-y divide-slate-100">
              {chapters.map((chapter) => (
                <div
                  ...
                  className="flex items-center gap-6 p-6 hover:bg-slate-50 transition-colors group cursor-pointer"
                >
                  {/* Chapter Icon */}
                  ...
                  {variants.map((variant) => (
                    <div key={variant.id} className="p-5 border border-slate-200 rounded-2xl bg-slate-50 flex items-center justify-between group">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          onClick={() => router.push(`/${currentLang}/variants/${variant.id}`)}
                          className="p-2 hover:bg-white text-slate-400 hover:text-indigo-600 rounded-lg transition-colors border border-transparent hover:border-slate-200"
