...
export class StoriesService {
  ...
  async exploreStories(query: ExploreQueryDto) {
        ...
        : query.trendWindow === 'yesterday'
          ...
          : query.trendWindow === 'week'
            ...
            : query.trendWindow === 'month'
              ...
              : null;
    // "yesterday" = khoảng 24–48h trước → cần chặn trên.
    const trendWindowEnd =
      query.trendWindow === 'yesterday' ? new Date(now.getTime() - 24 * 60 * 60 * 1000) : null;
    ...
    const where: Prisma.StoryWhereInput = {
      ...
      ...(query.trendWindow !== undefined && query.trendWindow !== 'all' && trendWindowStart
        ? {
            OR: [
              { createdAt: { gte: trendWindowStart, ...(trendWindowEnd ? { lt: trendWindowEnd } : {}) } },
              { updatedAt: { gte: trendWindowStart, ...(trendWindowEnd ? { lt: trendWindowEnd } : {}) } }
            ]
          }
        : {}),
  ...
  async getStoryDetail(slug: string) {
    ...
    await this.attachChapterHlsUrls(story.chapters);

    return this.serializeStory(story);
  }

  /**
   * Gắn `hlsUrl` (HlsAsset.playlistUrl, status='ready') vào từng chapter và variant.
   * Dùng 1 truy vấn gộp cho chapter + 1 cho variant để tránh N+1 (giống music.service).
   * hlsUrl = null nếu HLS chưa sẵn sàng. Không che theo quyền: segment đã mã hoá AES-128,
   * khoá lấy qua /hls/:type/:id/key đã gated entitlement.
   */
  private async attachChapterHlsUrls(chapters: any[]) {
    if (!Array.isArray(chapters) || chapters.length === 0) return;

    const chapterIds = chapters.map((c) => c.id).filter(Boolean);
    const variantIds = chapters.flatMap((c) =>
      Array.isArray(c.variants) ? c.variants.map((v: any) => v.id).filter(Boolean) : [],
    );

    const [chapterMap, variantMap] = await Promise.all([
      this.loadReadyHls('chapter', chapterIds),
      this.loadReadyHls('variant', variantIds),
    ]);

    for (const chapter of chapters) {
      chapter.hlsUrl = chapterMap.get(chapter.id) ?? null;
      if (Array.isArray(chapter.variants)) {
        for (const variant of chapter.variants) {
          variant.hlsUrl = variantMap.get(variant.id) ?? null;
        }
      }
    }
  }

  /** Map of assetId → ready HLS playlist URL cho assetType cho trước. */
