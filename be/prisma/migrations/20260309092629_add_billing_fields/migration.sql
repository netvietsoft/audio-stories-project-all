...
CREATE TABLE `users` (
    ...
    UNIQUE INDEX `users_paypal_customer_id_key`(`paypal_customer_id`),
...
CREATE TABLE `payments` (
    ...
    `provider` ENUM('STRIPE', 'VIETQR', 'PAYPAL', 'MANUAL') NOT NULL DEFAULT 'VIETQR',
    `provider_payment_id` VARCHAR(255) NULL,
    `provider_customer_id` VARCHAR(255) NULL,
    `status` ENUM('PENDING', 'PROCESSING', 'SUCCESS', 'SUCCEEDED', 'FAILED', 'CANCELLED', 'REFUNDED', 'PARTIALLY_REFUNDED', 'REQUIRES_ACTION', 'REQUIRES_PAYMENT_METHOD') NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE `webhook_events` (
    ...
    INDEX `webhook_events_provider_event_type_idx`(`provider`, `event_type`),
