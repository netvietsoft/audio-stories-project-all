...
### Đăng nhập Google (Spec B)
- **App** — LoginScreen thêm nút "Tiếp tục với Google" (divider "hoặc"): `google_sign_in` v7 lấy idToken → `POST /auth/google/mobile` → phiên y hệt login thường (access body + refresh Set-Cookie). User hủy popup → thoát im lặng, không báo lỗi.
- **App** — Mới: `GoogleAuth` (lib/api/google_auth.dart — wrapper plugin, fake được trong test), `AuthRepository.loginGoogle`, `AuthNotifier.loginWithGoogle`, const `ApiEnv.googleServerClientId` + `ApiEndpoints.authGoogleMobile`.
- **BE** (backend-port, cần deploy + `yarn install`) — Mới `POST /auth/google/mobile {idToken}`: verify bằng `google-auth-library` (audience = `GOOGLE_CLIENT_ID`), verify + gate email verified (`verifyGoogleIdToken`), tái dùng `upsertGoogleUser` (tự link account trùng email) + `issueTokens`. 401 token sai; 503 khi chưa cấu hình `GOOGLE_CLIENT_ID`.
- **BE** — Việc tay 1 lần: tạo Android OAuth client (package `com.netviet.novelverse` + SHA-1 debug keystore) trong project Google Cloud hiện có — xem spec §4.
- **Trạng thái 2026-07-18 — ĐÃ LIVE + DEVICE PASS**: final review 2 repo READY TO MERGE (app 84/84 test; BE 5/5 + build sạch). BE đã merge/deploy prod (user), `.env` VPS đã sửa 5 biến di sản foodsnear (GOOGLE_* ×3 + CLIENT_URL/ALLOWED_CLIENT_URLS → dreamtap — fix luôn web Google login trước đó redirect nhầm sang foodsnear BE). Android OAuth client đã tạo. Device pass A50s: login Google mới tạo user OK · hủy popup im lặng OK · logout→login lại OK (không consent lại). Chưa test trên máy: link account trùng email (logic đã unit-test), push history sau login (wiring đã review). Checklist tham chiếu: `D:\SetupC\Projects\NovelApp\GOOGLE-SIGNIN-SETUP-CHECKLIST.md`.
- **Bảo mật (fix cùng ngày, BE commit `5f26fca` — cần redeploy)**: (1) gate `email_verified` port vào `upsertGoogleUser` — đóng lỗ web flow nhận Google account email chưa xác thực (mobile double-gate); (2) chống account pre-hijack: account chưa-verify được auto-verify qua Google link → vô hiệu password cũ (`passwordHash = null`, đặt lại qua forgot-password). 5 unit test mock prisma; review xác minh không chặn nhầm account thật (OIDC userinfo luôn trả `email_verified`).
...
- **Banner carousel** (`GET /banners`, admin quản): tự trượt 5s + chấm trang, dưới header Reading; bấm: banner gắn truyện (`storySlug`) → mở chi tiết truyện trong app (`/book/<slug>`), không thì `targetUrl` → trình duyệt ngoài (`url_launcher`). Rỗng/lỗi → ẩn.
...
- **Thanh kéo mượt**: khi kéo chỉ cập nhật sheet (số hiển thị vẫn live), áp cỡ chữ/giãn dòng vào trang đọc + lưu khi **thả tay** — trước đó mỗi nấc kéo rebuild cả chương dài (1.700+ đoạn) gây giật.
...
- 3 khối load song song, lỗi khối nào fallback/ẩn khối đó; Editor's Pick + Trending có cache-first theo ngôn ngữ (kệ chủ đề fetch tươi — BE đã cache 1h phía server).
- Fix theo final review: **bỏ load đúp toàn bộ request lúc mở app** (initState + hook đổi-ngôn-ngữ cùng bắn — sửa luôn double-load Hot Ranking có từ trước); màn thể loại có hàng **"Thử lại"** khi lỗi tải trang kế (hết spinner quay vô hạn).
...
- **Support**: sheet quà giữ giao diện cũ nhưng tặng **Pulse thật** (`POST /stories/:id/gift`, kèm chương đang đọc + message tên quà); lỗi thiếu Pulse hiện từ BE; tặng xong tự đồng bộ số dư. `Book` thêm `uuid` (BE cần UUID, không nhận slug). Lưu ý: truyện **đã tải trước bản này** chưa có uuid trong meta offline → nút Support báo "Chưa đồng bộ truyện" cho tới khi tải lại/resume truyện đó.
