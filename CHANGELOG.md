...
- **Trạng thái 2026-07-18**: final review 2 repo READY TO MERGE (app 84/84 test; BE 5/5 + build sạch); APK đã cài A50s, nút Google render đúng. BE nằm ở nhánh `feat/reader-player-port` (đến `05fd89b`) — **CHƯA merge main/deploy**; tính năng chỉ chạy sau khi: (1) tạo Android OAuth client, (2) deploy BE + check `GOOGLE_CLIENT_ID` .env VPS trùng `ApiEnv.googleServerClientId`. Checklist đầy đủ: `D:\SetupC\Projects\NovelApp\GOOGLE-SIGNIN-SETUP-CHECKLIST.md`.
