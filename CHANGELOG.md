...
- **⚠ Bảo mật pre-existing (ngoài phạm vi đợt này, nên xử sớm)**: (1) web flow prod (`google.strategy → upsertGoogleUser`) THIẾU gate email_verified — mobile đã đóng, web chưa; (2) account pre-hijacking: đăng ký trước email nạn nhân → nạn nhân login Google auto-verify → password kẻ đăng ký trước dùng được — cân nhắc reset password khi auto-verify qua Google.
