...
export default function InteractiveStoriesPage() {
  ...
  const handleEditChapter = async (chapter: ChapterInfo) => {
    ...
    try {
      ...
      setFullChapterData(unwrapData<any>(res.data));
    } catch (error) {
  ...
  const handleEditVariant = async (variant: Variant) => {
    ...
    try {
      ...
      setEditingVariantData(unwrapData<any>(res.data));
    } catch (error) {
        ...
        chaptersRaw.map(async (ch: any) => {
          try {
            ...
            return {
              ...
              variants: unwrapList<VariantInfo>(varRes.data),
            };
          } catch {
            return {
              id: ch.id,
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      {/* Stories List */}
      {isLoading ? (
        <div className="bg-white rounded-[40px] border border-slate-200 shadow-xl shadow-slate-200/40 flex justify-center py-20">
          <Loader2 className="w-8 h-8 animate-spin text-amber-500" />
        </div>
      ) : filteredStories.length === 0 ? (
        <div className="bg-white rounded-[40px] border border-slate-200 shadow-xl shadow-slate-200/40 px-8 py-20 text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            {filteredStories.map((story) => {
              ...
              return (
                <div key={story.id} className="bg-white rounded-[32px] border border-slate-200 shadow-lg shadow-slate-200/40 overflow-hidden">
                  {/* Story Row */}
                  <div
                    role="button"
                    tabIndex={0}
