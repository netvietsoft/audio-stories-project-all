...
export default function CategoriesClient({ initialSlug }: { initialSlug?: string }) {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      ...
      const categoryList = unwrapList<CategoryItem>(res.data);
      const totalCount = categoryList.reduce((sum, cat) => sum + cat.storiesCount, 0);
      setCategories([{ id: 0, name: t("allCategories"), slug: "all", storiesCount: totalCount }, ...categoryList]);
    };
