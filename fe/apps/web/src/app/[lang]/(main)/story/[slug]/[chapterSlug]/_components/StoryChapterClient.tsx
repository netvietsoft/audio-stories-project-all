...
export default function StoryChapterClient() {
  ...
  const normalizeVariant = useCallback(
    (variant: any): ChapterVariant => {
      ...
      return {
        ...
        hasAudio: Boolean(rawAudioUrl || variant.hlsUrl || Number(variant.audioDuration) > 0),
      };
    },
    [locale, accessToken],
  );
  ...
  const normalizeChapter = useCallback(
    (chapter: any): ChapterItem => {
      ...
      return {
        ...
        hasAudio: Boolean((rawAudioUrl || "").trim() || chapter.hlsUrl || Number(chapter.audioDuration) > 0),
      };
    },
    [locale, normalizeVariant, accessToken],
  );

  // hlsUrl của track = variant.hlsUrl (nếu đang chọn variant) hoặc chapter.hlsUrl; null nếu HLS chưa sẵn sàng.
  // BE chỉ trả hlsUrl khi HlsAsset.status='ready'. Player tự fallback về audioUrl/proxy khi null.
  const resolveTrackHlsUrl = useCallback(
    (chapter: ChapterItem, variant?: ChapterVariant | null): string | null => {
      if (variant) return variant.hlsUrl ?? null;
      return chapter.hlsUrl ?? null;
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        // /stories/:slug is wrapped ({ data: {...story, chapters} }); unwrap to the real
        // object so detail.chapters is populated (else the reader shows "no version" pending).
        const detail = unwrapData<StoryDetail>(detailRes.data) as StoryDetail;
  ...
  useEffect(() => {
    ...
    const fetchChapterDetail = async () => {
      try {
        ...
        // /chapters/:id/public is wrapped ({ data: {...chapter} }); unwrap so content is read.
  ...
  */
  ...
  // Chỉ tính là "có audio" khi chương/variant thực sự có file (audioUrl/HLS) từ BE,
  // không tính proxy URL mặc định (r2AudioUrl luôn được gán proxy trong normalize).
  const hasPlayableAudio = selectedVariant
    ? Boolean(selectedVariant.hasAudio)
    : Boolean(selectedChapter?.hasAudio);
  ...
  useEffect(() => {
    if (!story || !selectedChapter || !hasPlayableAudio || !selectedChapterAudioUrl || !user) return;
    ...
    const restoreProgress = async () => {
      try {
        ...
        const resumeTrack = {
          ...
          hlsUrl: resolveTrackHlsUrl(selectedChapter, selectedVariant),
        ...
        setQueue(
          story.chapters.map((chapter) => ({
            ...
            hlsUrl: resolveTrackHlsUrl(chapter),
  ...
  useEffect(() => {
    ...
    const refreshedQueue = story.chapters.map((item) => ({
      ...
      hlsUrl: resolveTrackHlsUrl(item),
    ...
    setTrack({
      ...
      hlsUrl: resolveTrackHlsUrl(latestChapter),
      coverUrl: latestChapter.thumbnailUrl || story.thumbnailUrl || undefined,
      storyCoverUrl: story.thumbnailUrl || undefined,
    });
  }, [currentTrack, locale, resolveTrackHlsUrl, setQueue, setTrack, story, t]);
  ...
  const playChapter = useCallback(
    async (chapter: ChapterItem, selectedStory: StoryDetail, autoPlay = true, variantId?: string) => {
      ...
      const track = {
        ...
        hlsUrl: resolveTrackHlsUrl(chapter, targetVariant),
    ...
    [currentTime, isVipActive, locale, playTrack, resolveTrackHlsUrl, selectedChapterId, selectedVariant, selectedVariantId, setQueue, setTrack, t, togglePlay],
  );
    ...
    <div className="w-full space-y-3 md:space-y-4">
      <div className="flex flex-col gap-3 lg:grid lg:grid-cols-[minmax(0,1fr)_380px] xl:grid-cols-[minmax(0,1fr)_420px] 2xl:grid-cols-[minmax(0,1fr)_460px] lg:items-start">
        ...
        <div className="lg:col-start-2 lg:row-start-1 lg:row-end-6 relative lg:sticky lg:top-24 h-fit flex flex-col gap-3 z-10">
          ...
          {/* Side Panel 2: Audio Player (chỉ hiện khi chương có audio) */}
          {hasPlayableAudio && (
          <div className="-mx-4 md:mx-0 md:bg-transparent lg:bg-transparent">
            <div className="px-4 md:px-0">
