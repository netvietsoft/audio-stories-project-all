...
export default function AdForm({ initialData, isLoading, showUnlockAdvanced = false, onSubmit, onCancel }: AdFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(internalOnSubmit, handleFormError)} className="w-full space-y-8">
          ...
          {contentType === 'image' ? (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="space-y-2">
                ...
                {/* >>> KÍCH THƯỚC ẢNH PREVIEW: previewWidthClass="w-1/2" = 1/2 bề rộng (≈1/4 diện tích so với w-full). Đổi w-1/2 -> w-full / w-1/3 để chỉnh. */}
                <HybridImageUploader
                  value={previewImage}
