  ...
  useEffect(() => {
    const fetchFrequencyConfig = async () => {
      ...
      try {
        ...
        const rawValue = unwrapData<any>(response.data)?.value;
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                ...
                items.map((item) => (
                  <tr
                    key={item.id}
                    onClick={() => router.push(`/${params?.lang === 'en' ? 'en' : 'vi'}/ads/${item.id}`)}
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/ads/${item.id}`} onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 transition hover:bg-slate-100">
