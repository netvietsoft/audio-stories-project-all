...
export class ChapterVariantsService {
  ...
  /** Enqueue per-variant HLS transcode when the variant has audio (after commit). */
  private async registerVariantHls(variant: {
    id: string;
  ...
  async create(data: CreateChapterVariantDto) {
    const variant = await this.prisma.chapterVariant.create({
      data,
    });
    await this.registerVariantHls(variant);
    return variant;
  }

  async update(id: string, data: UpdateChapterVariantDto) {
    await this.findOne(id);
    const variant = await this.prisma.chapterVariant.update({
      where: { id },
      data,
    });
    const audioChanged =
      (data as { audioUrl?: unknown }).audioUrl !== undefined ||
      (data as { r2AudioUrl?: unknown }).r2AudioUrl !== undefined;
    if (audioChanged) await this.registerVariantHls(variant);
    return variant;
  }
