...
export type HomePageInitialData = {
  newestStories: StoryItem[];
  newestChapters: any[];
  popularStories: StoryItem[];
  completedStories: StoryItem[];
  trendingStories: StoryItem[];
  actionStories: StoryItem[];
  xuyenKhongStories: StoryItem[];
  shounenStories: StoryItem[];
  tienHiepStories: StoryItem[];
  topCategories: CategoryItem[];
  allCategories: CategoryItem[];
  authors: AuthorItem[];
  hallContributors: HallContributor[];
  heroBanners: BannerItem[];
  displayCategories: CategoryItem[];
  displayCategoryStories: Record<number, StoryItem[]>;
};
...
export default function HomePageClient({ initialData }: { initialData: HomePageInitialData }) {
  ...
  const isInitialClientRefreshRef = useRef(true);

  const [newestStories, setNewestStories] = useState<StoryItem[]>(initialData.newestStories || []);
  const [newestChapters, setNewestChapters] = useState<any[]>(initialData.newestChapters || []);
  const [popularStories, setPopularStories] = useState<StoryItem[]>(initialData.popularStories || []);
  const [completedStories, setCompletedStories] = useState<StoryItem[]>(initialData.completedStories || []);
  const [trendingStories, setTrendingStories] = useState<StoryItem[]>(initialData.trendingStories || []);
  const [actionStories, setActionStories] = useState<StoryItem[]>(initialData.actionStories || []);
  const [xuyenKhongStories, setXuyenKhongStories] = useState<StoryItem[]>(initialData.xuyenKhongStories || []);
  const [shounenStories, setShounenStories] = useState<StoryItem[]>(initialData.shounenStories || []);
  const [tienHiepStories, setTienHiepStories] = useState<StoryItem[]>(initialData.tienHiepStories || []);
  const [topCategories, setTopCategories] = useState<CategoryItem[]>(initialData.topCategories || []);
  const [allCategories, setAllCategories] = useState<CategoryItem[]>(initialData.allCategories || []);
  const [authors, setAuthors] = useState<AuthorItem[]>(initialData.authors || []);
  const [hallContributors, setHallContributors] = useState<HallContributor[]>(initialData.hallContributors || []);
  const [heroBanners, setHeroBanners] = useState<BannerItem[]>(initialData.heroBanners || []);
  const [favoriteStories, setFavoriteStories] = useState<FavoriteItem[]>([]);
  const [historyItems, setHistoryItems] = useState<HistoryItem[]>([]);
  
  const [displayCategories, setDisplayCategories] = useState<CategoryItem[]>(initialData.displayCategories || []);
  const [displayCategoryStories, setDisplayCategoryStories] = useState<Record<number, StoryItem[]>>(initialData.displayCategoryStories || {});
  ...
  useEffect(() => {
    const loadHome = async () => {
      if (!isInitialClientRefreshRef.current) {
        setIsLoading(true);
      }
      try {
        const [
        ...
        const catTop = catTopRes.status === "fulfilled" ? unwrapList<CategoryItem>(catTopRes.value) : [];
        const catFb = catFallbackRes.status === "fulfilled"
          ? unwrapList<any>(catFallbackRes.value).map((c) => ({ ...c, storiesCount: 0 }))
          : [];
        ...
        setAuthors(authorRes.status === "fulfilled" ? unwrapList<AuthorItem>(authorRes.value) : []);
        setHallContributors(hallRes.status === "fulfilled" ? unwrapList<HallContributor>(hallRes.value) : []);
        setHeroBanners(bannersRes.status === "fulfilled" ? unwrapList<BannerItem>(bannersRes.value) : []);
      } catch (error) {
        console.error(t("loadError"), error);
      } finally {
        if (isInitialClientRefreshRef.current) {
          isInitialClientRefreshRef.current = false;
        }
        setIsLoading(false);
      }
    };
