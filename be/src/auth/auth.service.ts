...
export class AuthService {
  ...
  async loginLocal(email: string, password: string, ip?: string): Promise<TokenPair> {
    ...
    // H7: chặn đăng nhập khi tài khoản bị khoá (isActive=false). Dùng cùng message
    // với credential-fail để không lộ tài khoản tồn tại hay không.
  ...
  /** Đổi mật khẩu khi đã đăng nhập → thu hồi mọi phiên khác (buộc đăng nhập lại). */
  async changePassword(userId: string, currentPassword: string, newPassword: string): Promise<{ ok: boolean }> {
    const result = await this.passwordService.changePassword(userId, currentPassword, newPassword);
  ...
  /**
   * H7: khoá/mở khoá tài khoản (admin). Khi khoá (isActive=false) đồng thời thu
   * hồi mọi refresh token để phiên đang mở không thể gia hạn — access token còn
   * hạn cũng bị JwtAccessStrategy.validate từ chối (kiểm isActive).
   */
  async setUserActive(userId: string, isActive: boolean) {
    ...
    return {
      success: true,
      message: isActive ? 'User unlocked' : 'User locked',
      data: { userId: updated.id, isActive: updated.isActive },
    };
  }
}
