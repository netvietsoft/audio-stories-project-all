  ...
  useEffect(() => {
    const loadStories = async () => {
      ...
      try {
        ...
        // /stories/explore is double-wrapped ({ data: { data: [...] } }) via the
        // global interceptor; extract the array defensively regardless of wrap depth.
        const raw: any = response.data;
        const rows: StoryItem[] = Array.isArray(raw)
          ? raw
        ...
        const unique = Array.from(
          new Map(rows.map((story) => [story.id, story])).values(),
        );
