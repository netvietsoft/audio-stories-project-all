...
export class WebhookService {
  ...
  private async handleCheckoutCompleted(session: any) {
    ...
    try {
      ...
      if (error?.code === 'P2002') {
        // Unique (provider, providerPaymentId) violation — the verify-payment path
        // already credited this Checkout Session. Ignore; prevents double credit.
        this.logger.log('Checkout already processed (unique constraint) — skipping');
        return;
      }
