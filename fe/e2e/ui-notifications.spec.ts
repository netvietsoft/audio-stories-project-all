...
test.describe("Notifications UI E2E", () => {
  ...
  test("Trang danh sách: render đầy đủ, gọi read-all API, badge biến mất và item chuyển style đã đọc", async ({
    ...
    await page.goto("/");
    const homeBellBefore = page.locator("button").filter({ has: page.locator("svg.lucide-bell") }).first();
    await expect(homeBellBefore.locator("span.bg-red-500")).toBeVisible();

    // Dự án hiện có route /notifications; ưu tiên /profile/notifications theo yêu cầu,
    // nhưng fallback nếu route này chưa được map hoặc đang hiển thị trang not-found.
    await page.goto("/profile/notifications");
    const hasNotificationsHeading = await page.getByText(/Thong bao/i).first().isVisible().catch(() => false);
    const hasMarkAllAction = await page
      .getByRole("button", { name: /Danh dau da doc tat ca|Đánh dấu tất cả đã đọc/i })
      .first()
      .isVisible()
      .catch(() => false);

    if (!hasNotificationsHeading || !hasMarkAllAction) {
    ...
    const notif1Container = page.getByText("Thong bao 1").locator("xpath=ancestor::div[contains(@class,'rounded-xl')][1]");
    const notif2Container = page.getByText("Thong bao 2").locator("xpath=ancestor::div[contains(@class,'rounded-xl')][1]");
  ...
  await page.goto("/");
  const homeBellAfter = page.locator("button").filter({ has: page.locator("svg.lucide-bell") }).first();
