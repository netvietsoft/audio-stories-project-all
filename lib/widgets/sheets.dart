...
/// Popup bình luận: danh sách + ô nhập (prototype, chỉ thêm vào list cục bộ).
Future<void> showCommentSheet(BuildContext context) {
  return _showSheet<void>(context, (c) {
    final pal = context.pal;
    final comments = <(String, String)>[
      ('Reader_22', 'Hooked from chapter 1! 😍'),
      ('NovelFan', 'Plot twist in ch.24 was insane.'),
      ('Mira', 'Please update faster 🙏'),
      ('Leo', 'Best romance this year.'),
    ];
    ...
    return StatefulBuilder(builder: (c, setSheet) {
      return Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Comments (${comments.length})', style: AppType.section(color: pal.ink)),
          const SizedBox(height: Gap.md),
          Flexible(
            child: ListView(shrinkWrap: true, children: [
              for (final (user, text) in comments)
                Container(
                  margin: const EdgeInsets.only(bottom: Gap.sm),
                  padding: const EdgeInsets.all(Gap.md),
                  decoration: BoxDecoration(color: pal.surf2, borderRadius: rounded(12)),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(user, style: AppType.item(size: 12.5, color: pal.ink)),
                    const SizedBox(height: 3),
                    Text(text, style: AppType.body(size: 13, color: pal.soft)),
                  ]),
                ),
            ]),
          ),
            ...
            Expanded(
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  border: OutlineInputBorder(borderRadius: rounded(22), borderSide: BorderSide.none),
                ),
              ),
            ),
            IconButton(
              icon: const Icon(Icons.send_rounded, color: AppPalette.terracotta),
              onPressed: () {
                if (ctrl.text.trim().isEmpty) return;
                setSheet(() {
                  comments.insert(0, ('You', ctrl.text.trim()));
                  ctrl.clear();
                });
              },
            ),
